...
public class PathFinder {
		...
		for (int i = 0; i < savedRailBases.size() - 1; i++) {
			...
			if (savedRailBaseStart.transportMode == TransportMode.AIRPLANE) {
				rails.forEach((startPos, railMap) -> {
					// One-way rails also record the blocked direction as a RailType.NONE placeholder, so a runway end
					// that anything can taxi into always carries a second entry here. Counting those made every
					// reachable runway invisible, left this set empty, and no flight leg was ever built.
					if (countUsable(railMap) == 1 && railMap.values().stream().anyMatch(rail -> rail.railType == RailType.RUNWAY)) {
	...
	private static void addPathPart(Map<BlockPos, Map<BlockPos, Rail>> rails, Set<BlockPos> runways, BlockPos newPos, BlockPos lastPos, List<PathPart> path, Set<BlockPos> turnBacks, Function<Map<BlockPos, Rail>, Comparator<BlockPos>> comparator) {
		...
		// Every position used to be present by construction, so this was safe to dereference. Filtering the map by
		// train length breaks that: a node whose every rail refuses the train is dropped outright, and the very
		// first call comes from the departure rail base itself — a platform that only takes short trains is exactly
		// the case a builder tries first. The early return below already handles a null oldRail properly.
		final Map<BlockPos, Rail> lastConnections = rails.get(lastPos);
		final Rail oldRail = lastConnections == null ? null : lastConnections.get(newPos);
		...
		if (newConnections != null) {
			...
			if (oldRail != null && oldRail.railType == RailType.RUNWAY && countUsable(newConnections) <= 1) {
	...
	/** Rails a vehicle can actually depart along, ignoring the RailType.NONE placeholders that one-way rails leave behind. */
	private static long countUsable(Map<BlockPos, Rail> railMap) {
		return railMap == null ? 0 : railMap.values().stream().filter(rail -> rail.railType != RailType.NONE).count();
	}
