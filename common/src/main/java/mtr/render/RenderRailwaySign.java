...
public class RenderRailwaySign<T extends BlockRailwaySign.TileEntityRailwaySign> extends BlockEntityRendererMapper<T> implements IBlock, IGui, IDrawing {
	...
	private static int cachedSignGeneration = -1;
	// getSign is called upwards of a hundred times per sign block per frame (once per sign, plus twice more from
	// each getMaxWidth scan). Resolving it built a new CustomSign every time, and every custom sign id also threw
	// and caught an IllegalArgumentException out of SignType.valueOf, which dominated the cost.
	private static final Map<String, CustomResources.CustomSign> SIGN_CACHE = new HashMap<>();
	...
	public void render(T entity, float tickDelta, PoseStack matrices, MultiBufferSource vertexConsumers, int light, int overlay) {
		...
		if (renderBackground) {
			...
			RenderTrains.scheduleRender(WHITE_TEXTURE, false, RenderTrains.QueuedRenderLayer.LIGHT, (matricesNew, vertexConsumer) -> {
		...
		for (int i = 0; i < signIds.length; i++) {
			if (signIds[i] != null) {
				drawSign(matrices, vertexConsumers, storedMatrixTransformations, Minecraft.getInstance().font, pos, signIds[i], 0.5F * i, 0, 0.5F, getMaxWidth(signIds, i, false), getMaxWidth(signIds, i, true), entity.getSelectedIds(), facing, backgroundColor | ARGB_BLACK, (textureId, x, y, size, flipTexture) -> {
					RenderTrains.scheduleRender(textureId, true, RenderTrains.QueuedRenderLayer.LIGHT_TRANSLUCENT, (matricesNew, vertexConsumer) -> {
						storedMatrixTransformations.transform(matricesNew);
	...
	public static CustomResources.CustomSign getSign(String signId) {
		...
		if (cachedSignGeneration != RenderTrains.getTextureGeneration()) {
			cachedSignGeneration = RenderTrains.getTextureGeneration();
			SIGN_CACHE.clear();
		}
		if (SIGN_CACHE.containsKey(signId)) {
			return SIGN_CACHE.get(signId);
		}
		final CustomResources.CustomSign customSign = resolveSign(signId);
		SIGN_CACHE.put(signId, customSign);
		return customSign;
	}

	private static CustomResources.CustomSign resolveSign(String signId) {
		for (final BlockRailwaySign.SignType sign : BlockRailwaySign.SignType.values()) {
			if (sign.name().equals(signId)) {
