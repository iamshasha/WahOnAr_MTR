...
public class RenderPSDAPGDoor<T extends BlockPSDAPGDoorBase.TileEntityPSDAPGDoorBase> extends BlockEntityRendererMapper<T> implements IGui, IBlock {
	...
	// Texture paths only depend on block state flags, so resolve them once instead of formatting a string every frame.
	private static final ResourceLocation TEXTURE_DOOR_NOT_IN_USE = new ResourceLocation("mtr:textures/block/sign/door_not_in_use.png");
	private static final ResourceLocation[] TEXTURE_PSD_LIGHT = {
			new ResourceLocation("mtr:textures/block/light_off.png"),
			new ResourceLocation("mtr:textures/block/light_on.png")
	};
	private static final ResourceLocation[] TEXTURE_APG_LIGHT = {
			new ResourceLocation("mtr:textures/block/apg_door_light_off.png"),
			new ResourceLocation("mtr:textures/block/apg_door_light_on.png")
	};
	// Indexed by [half ? 1 : 0][side ? 1 : 0][type == 1 ? 1 : 0]
	private static final ResourceLocation[][][] TEXTURE_PSD_DOOR = buildPsdTextures("mtr:textures/block/psd_door_%s_%s_%s.png");
	private static final ResourceLocation[][][] TEXTURE_PSD_DOOR_END_1 = buildPsdTextures("mtr:textures/block/psd_door_end_%s_%s_1_%s.png");
	private static final ResourceLocation[][][] TEXTURE_PSD_DOOR_END_2 = buildPsdTextures("mtr:textures/block/psd_door_end_%s_%s_2_%s.png");
	// Indexed by [half ? 1 : 0][side ? 1 : 0]
	private static final ResourceLocation[][] TEXTURE_APG_DOOR = buildHalfSideTextures("mtr:textures/block/apg_door_%s_%s.png");
	private static final ResourceLocation[][] TEXTURE_LIFT_DOOR = buildHalfSideTextures("mtr:textures/block/lift_door_%s_%s_1.png");

	private static ResourceLocation[][][] buildPsdTextures(String format) {
		final ResourceLocation[][][] textures = new ResourceLocation[2][2][2];
		for (int half = 0; half < 2; half++) {
			for (int side = 0; side < 2; side++) {
				for (int type = 0; type < 2; type++) {
					textures[half][side][type] = new ResourceLocation(String.format(format, half == 1 ? "top" : "bottom", side == 1 ? "right" : "left", type == 1 ? "2" : "1"));
				}
			}
		}
		return textures;
	...
	private static ResourceLocation[][] buildHalfSideTextures(String format) {
		final ResourceLocation[][] textures = new ResourceLocation[2][2];
		for (int half = 0; half < 2; half++) {
			for (int side = 0; side < 2; side++) {
				textures[half][side] = new ResourceLocation(String.format(format, half == 1 ? "top" : "bottom", side == 1 ? "right" : "left"));
			}
		}
		return textures;
	}
	...
	public void render(T entity, float tickDelta, PoseStack matrices, MultiBufferSource vertexConsumers, int light, int overlay) {
		...
		// Before any block state is read: a platform's worth of screen doors is a few hundred block entities, and
		// every one of them was reading five properties and queueing geometry whether or not it faced the camera.
		if (RenderTrains.shouldNotRenderBlockDetail(pos, null)) {
			return;
		}
		...
		switch (type) {
			...
			case 1:
				if (half) {
					RenderTrains.scheduleRender(TEXTURE_PSD_LIGHT[open > 0 ? 1 : 0], false, open > 0 ? RenderTrains.QueuedRenderLayer.LIGHT : RenderTrains.QueuedRenderLayer.EXTERIOR, (matricesNew, vertexConsumer) -> {
				...
				if (end) {
					RenderTrains.scheduleRender(TEXTURE_PSD_DOOR_END_2[half ? 1 : 0][side ? 1 : 0][type == 1 ? 1 : 0], false, RenderTrains.QueuedRenderLayer.EXTERIOR, (matricesNew, vertexConsumer) -> {
						storedMatrixTransformationsLight.transform(matricesNew);
			...
			case 2:
				if (half) {
					...
					if (block instanceof BlockAPGGlass || block instanceof BlockAPGGlassEnd) {
						RenderTrains.scheduleRender(TEXTURE_APG_LIGHT[open > 0 ? 1 : 0], false, open > 0 ? RenderTrains.QueuedRenderLayer.LIGHT_TRANSLUCENT : RenderTrains.QueuedRenderLayer.EXTERIOR, (matricesNew, vertexConsumer) -> {
							storedMatrixTransformationsLight.transform(matricesNew);
		...
		switch (type) {
			...
			case 1:
				if (end) {
					RenderTrains.scheduleRender(TEXTURE_PSD_DOOR_END_1[half ? 1 : 0][side ? 1 : 0][type == 1 ? 1 : 0], false, RenderTrains.QueuedRenderLayer.EXTERIOR, (matricesNew, vertexConsumer) -> {
					...
					RenderTrains.scheduleRender(TEXTURE_PSD_DOOR[half ? 1 : 0][side ? 1 : 0][type == 1 ? 1 : 0], false, RenderTrains.QueuedRenderLayer.EXTERIOR, (matricesNew, vertexConsumer) -> {
						storedMatrixTransformations.transform(matricesNew);
				...
				if (half && !unlocked) {
					RenderTrains.scheduleRender(TEXTURE_DOOR_NOT_IN_USE, false, RenderTrains.QueuedRenderLayer.EXTERIOR, (matricesNew, vertexConsumer) -> {
						storedMatrixTransformations.transform(matricesNew);
						if (end) {
				...
				break;
			case 2:
				RenderTrains.scheduleRender(TEXTURE_APG_DOOR[half ? 1 : 0][side ? 1 : 0], false, RenderTrains.QueuedRenderLayer.EXTERIOR, (matricesNew, vertexConsumer) -> {
					storedMatrixTransformations.transform(matricesNew);
			...
			case 3:
				RenderTrains.scheduleRender(TEXTURE_LIFT_DOOR[half ? 1 : 0][side ? 1 : 0], false, RenderTrains.QueuedRenderLayer.EXTERIOR, (matricesNew, vertexConsumer) -> {
					storedMatrixTransformations.transform(matricesNew);
