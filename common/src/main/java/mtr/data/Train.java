...
public abstract class Train extends NameColorDataBase implements IPacket {
	...
	/**
	 * Whether this train opens its doors at a stop with nothing built beside it.
	 *
	 * Pushed in from the siding each tick rather than carried in the train's own saved data: the addon that
	 * replaces the movement loop also injects into three of this class's constructors and into its serialisation
	 * by exact descriptor, so both are the wrong places to add anything.
	 */
	protected boolean doorsWithoutPlatform;
	...
	// ponytail: a flat cap on the lookahead scan; braking distance normally stops it far sooner
	private static final int MAX_SPEED_LIMIT_LOOKAHEAD = 64; // m/tick^2
	...
	public final float getRailSpeed(int railIndex) {
		...
		// Take the lowest limit the train can already see, so it brakes on approach rather than at the board. A
		// train behind its booked time may run past the limits a builder set, but never past what the rail type
		// itself allows, and never past the room it has behind the train in front.
		final float allowed = Math.max(getSpeedLimitAhead(railIndex), getCatchUpSpeed());
		return Math.min(railSpeed, Math.min(allowed, getFollowingSpeedLimit()));
	}

	/**
	 * How fast the train has to run to make up time it has already lost, in blocks per tick, or 0 if it is not behind.
	 *
	 * Only the server knows what a train is booked to do, so the default is no catch-up at all.
	 */
	...
	/**
	 * How fast the train may run given how close the train in front is, in blocks per tick.
	 *
	 * Only the server knows where other trains are, so the default is no limit at all. Feeding it through the rail
	 * speed keeps the braking in the one place that already handles "moving faster than allowed", which also means
	 * addons that replace the movement loop but still ask for a rail speed inherit the behaviour.
	 */
	...
	/**
	 * The tightest per-rail speed limit within braking distance, in blocks per tick.
	 *
	 * Looking ahead here rather than in the movement code means the ordinary "faster than the limit, so slow down"
	 * branch does the braking, and it keeps working under addons that replace the movement loop but still call this.
	 */
	private float getSpeedLimitAhead(int railIndex) {
		float limit = Float.MAX_VALUE;
		final double brakingDistance = accelerationConstant <= 0 ? 0 : speed * speed / (2 * accelerationConstant);

		for (int i = railIndex; i < path.size() && i < railIndex + MAX_SPEED_LIMIT_LOOKAHEAD; i++) {
			final int railLimit = path.get(i).rail.speedLimitKmh;
			if (railLimit > 0) {
				limit = Math.min(limit, railLimit / 3.6F / 20);
			}
			// Everything past the point the train could still stop by is not its problem yet
			if (i > railIndex && i - 1 < distances.size() && distances.get(i - 1) - railProgress > brakingDistance) {
				break;
			}
		}
	...
	/** The leading edge of the train along its path, in world space. Null if the path cannot be read yet. */
	protected final Vec3 getHeadPosition() {
		try {
			return getRoutePosition(0, spacing);
		} catch (Exception e) {
			return null;
		}
	}

	/** The trailing edge of the train along its path, in world space. Null if the path cannot be read yet. */
	protected final Vec3 getTailPosition() {
		try {
			return getRoutePosition(trainCars, spacing);
		} catch (Exception e) {
			return null;
		}
	}
	...
	private boolean scanDoors(Level world, double trainX, double trainY, double trainZ, float checkYaw, float pitch, double halfSpacing, int dwellTicks) {
		...
		// Nothing to open against, but the siding says to open anyway. Both sides, because with no platform found
		// there is nothing to say which side the passengers are on.
		return hasPlatform || doorsWithoutPlatform;
	}
