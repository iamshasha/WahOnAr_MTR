...
public class Siding extends SavedRailBase implements IPacket, IReducedSaveData {
	...
	/** Cars to spawn from this siding. 0 means as many as the rail length allows. */
	private int maxTrainCars;
	/** Whether trains from this siding open their doors at a stop with no platform block or screen door beside it. */
	private boolean doorsWithoutPlatform;
	...
	/** False until this siding has run once since loading. Not saved; it is about this run of the server only. */
	private boolean clearedOnLoad;
	...
	private static final String KEY_DOORS_WITHOUT_PLATFORM = "doors_without_platform";
	...
	public Siding(Map<String, Value> map) {
		...
		doorsWithoutPlatform = messagePackHelper.getBoolean(KEY_DOORS_WITHOUT_PLATFORM);
	...
	public Siding(CompoundTag compoundTag) {
		...
		doorsWithoutPlatform = compoundTag.getBoolean(KEY_DOORS_WITHOUT_PLATFORM);
	...
	public Siding(FriendlyByteBuf packet) {
		...
		doorsWithoutPlatform = packet.readBoolean();
	...
	public void toReducedMessagePack(MessagePacker messagePacker) throws IOException {
		...
		messagePacker.packString(KEY_DOORS_WITHOUT_PLATFORM).packBoolean(doorsWithoutPlatform);
	...
	public void writePacket(FriendlyByteBuf packet) {
		...
		packet.writeBoolean(doorsWithoutPlatform);
	...
	public void setUnlimitedTrains(boolean unlimitedTrains, int maxTrains, int maxTrainCars, boolean doorsWithoutPlatform, boolean isManual, int maxManualSpeed, float accelerationConstant, int newDwellTime, boolean clearTrains, Consumer<FriendlyByteBuf> sendPacket) {
		final FriendlyByteBuf packet = new FriendlyByteBuf(Unpooled.buffer());
		packet.writeLong(id);
		packet.writeUtf(transportMode.toString());
		...
		this.maxTrainCars = maxTrainCars;
	...
	public void simulateTrain(DataCache dataCache, RailwayDataDriveTrainModule railwayDataDriveTrainModule, List<Map<UUID, Long>> trainPositions, SignalBlocks signalBlocks, Map<Player, Set<TrainServer>> trainsInPlayerRange, Set<TrainServer> trainsToSync, Map<Long, List<ScheduleEntry>> schedulesForPlatform, Map<Long, Map<BlockPos, TrainDelay>> trainDelays) {
		...
		for (final TrainServer train : trains) {
			train.setDoorsWithoutPlatform(doorsWithoutPlatform);
		...
		if (trainCars > 0 && (trains.isEmpty() || spawnTrain && (unlimitedTrains || trains.size() <= maxTrains))) {
			final TrainServer train = new TrainServer(unlimitedTrains || maxTrains > 0 ? new Random().nextLong() : id, id, railLength, trainId, baseTrainType, effectiveTrainCars(), path, distances, repeatIndex1, repeatIndex2, accelerationConstant, timeSegments, isManual, maxManualSpeed, dwellTime);
			trains.add(train);
		}
	...
	/**
	 * Sends any train that was mid-route when the server stopped back to the siding, once, on the first tick.
	 *
	 * A train reloaded halfway along its path has no booked departure and no way to work out which one it was
	 * running, so it either sits where it is or turns up at a time nothing expects. Clearing them was already the
	 * advice — there is a button for it in the depot screen — and no depot should depend on someone remembering to
	 * press it after every restart.
	 */
	private void clearStrandedTrainsOnLoad() {
		if (clearedOnLoad) {
			return;
		}
		clearedOnLoad = true;
	...
	/**
	 * Sends every train that is out on the route back to its siding, leaving anything already stabled alone.
	 *
	 * This is what both the automatic clear on server start and the depot's own Clear Trains button do. A train
	 * mid-route has no way to work out where it was going; a train sitting in a siding is exactly where it should
	 * be, and throwing it away was never what anyone pressing that button wanted.
	 */
	public void clearStrandedTrains() {
		trains.removeIf(TrainServer::getIsOnRoute);
	}
	...
	/**
	 * What the depot's Clear Trains button does: recover the trains that are lost, leave the ones that are not.
	 *
	 * It used to empty the siding outright, which meant a button offered as a way to unstick a stuck route also
	 * destroyed every train quietly waiting to be dispatched. It now does the same thing the automatic clear on
	 * server start does.
	 */
	...
	/**
	 * {@link #trainCars} clamped by the depot's ceiling, when that depot sets one. Read at spawn rather than baked into
	 * trainCars so that changing the depot's limit takes effect without having to regenerate every siding under it.
	 */
	public int effectiveTrainCars() {
		int cars = trainCars;
		if (maxTrainCars > 0) {
			cars = Math.min(cars, maxTrainCars);
		}
		if (depot != null && depot.maxTrainCars > 0) {
			cars = Math.min(cars, depot.maxTrainCars);
		}
		return cars;
	}

	/** Cars this siding is set to spawn, 0 meaning "as many as fit". Read by the siding screen. */
	/**
	...
	public int getDispatchLeadMillis() {
		...
		float travelTicks = 0;
		if (fromThisSiding != null) {
			for (final float ticks : fromThisSiding.values()) {
	...
	private int getFirstPlatformDwellTicks() {
		for (final PathData pathData : path) {
			if (pathData.savedRailBaseId != 0 && pathData.savedRailBaseId != id && pathData.dwellTime > 0) {
				return pathData.dwellTime * 10;
			}
		}
		return 0;
	}

	public boolean getDoorsWithoutPlatform() {
		return doorsWithoutPlatform;
	}

	public int getMaxTrainCars() {
		return maxTrainCars;
	}

	/** How many cars actually fit on this siding's rail, which is the ceiling the screen offers. */
	public int getMaxPossibleTrainCars() {
		final int spacing = TrainType.getSpacing(baseTrainType);
		return spacing <= 0 ? 0 : Math.min(transportMode.maxLength, (int) Math.floor(railLength / spacing));
	}

	public int getTrainSpacing() {
		return TrainType.getSpacing(baseTrainType);
	}
	...
	private void generateDefaultPath(Map<BlockPos, Map<BlockPos, Rail>> rails) {
		...
		trains.add(new TrainServer(id, id, railLength, trainId, baseTrainType, effectiveTrainCars(), path, distances, repeatIndex1, repeatIndex2, accelerationConstant, timeSegments, isManual, maxManualSpeed, dwellTime));
	}
	...
	private void generateTimeSegments(List<PathData> path, List<TimeSegment> timeSegments, Map<Long, Map<Long, Float>> platformTimes) {
		...
		final int spacing = TrainType.getSpacing(baseTrainType);
		double railProgress = (railLength + effectiveTrainCars() * spacing) / 2;
		...
		for (int i = 0; i < path.size(); i++) {
			...
			if (i + 1 < path.size() && pathData.isOppositeRail(path.get(i + 1))) {
				railProgress += spacing * effectiveTrainCars();
			}
		}
	}
