...
public class PacketTrainDataGuiServer extends PacketTrainDataBase {
	...
	/**
	 * Takes the depot and how far its path got, and resolves the position here rather than accepting coordinates from
	 * the client, so a caller cannot use this to teleport itself somewhere of its own choosing.
	 */
	public static void teleportToPathFailureC2S(MinecraftServer minecraftServer, ServerPlayer player, FriendlyByteBuf packet) {
		if (RailwayData.hasNoPermission(player)) {
			return;
		}
		...
		minecraftServer.execute(() -> {
			...
			final BlockPos pos = depot.getPathFailurePos(railwayData.dataCache, successfulSegments);
			if (pos == null) {
				player.displayClientMessage(Text.translatable("gui.mtr.path_failure_not_found"), true);
			} else {
